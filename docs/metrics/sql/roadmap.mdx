...
`mehen diff` directly (any `sql.*` key is a valid selector and threshold target). Named profile
presets (`sql.analytics_default`, `sql.migration_default`, `sql.procedural_default`) and
diff-aware delta gates remain to be wired through the threshold engine.
