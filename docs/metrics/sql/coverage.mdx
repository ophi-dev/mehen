...
Coverage for SQL files attaches at the file root. mehen's per-function coverage breakdown
applies to function-shaped scopes, and a SQL file's child scopes are statements, not functions —
so rank and gate SQL coverage at file granularity.
