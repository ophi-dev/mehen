...
[`mehen.toml` thresholds](/configuration), rendered in the JSON and Markdown reports of
[`mehen metrics`](/commands/metrics), and carried as per-file **trend columns** in
[`mehen diff`](/commands/diff#coverage-trend-columns) (base side supplied via `--base-coverage`,
retrieved for you by the [GitHub Action](/guides/github-action#coverage-trends)).

Coverage answers a question no static metric can: *which of this code do the tests actually
exercise?* Using [GitHub Code Quality](/guides/github-action#works-with-github-code-quality)'s
built-in coverage checks? The same Cobertura report feeds both — see the action guide for the
shared setup. Combined with complexity it locates the riskiest code in a repository — complex **and**
