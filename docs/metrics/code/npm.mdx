...
| C# | A method, constructor, operator, property, or indexer declared `public`. A member with no access modifier is `private`, and `internal` is assembly-scoped, so neither counts. Interface members are always public. |
