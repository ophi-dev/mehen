...
| **C#** | `.cs`, `.csx` | [ANTLR](https://www.antlr.org/) — [grammars-v4 C# grammar](https://github.com/antlr/grammars-v4/tree/master/csharp) via [antlr-rust-runtime](https://github.com/ophi-dev/antlr-rust-runtime) |
...
- **ANTLR** parses Kotlin, Java, and C#, generated to Rust by
  ...
  modules). C# uses the [grammars-v4 C# grammar](https://github.com/antlr/grammars-v4/tree/master/csharp),
  covering properties and accessors, indexers, events, local functions, expression-bodied members,
  pattern matching with `when` guards, interpolated strings, and `#if` preprocessor directives. All
  three are richer than a tree-sitter CST.
