...
- **ANTLR** parses Kotlin from the official [Kotlin specification grammar](https://github.com/Kotlin/kotlin-spec),
  generated to Rust by [`ophi-dev/antlr-rust-runtime`](https://github.com/ophi-dev/antlr-rust-runtime).
  The spec grammar models Kotlin constructs (`when` entries, the elvis `?:` and safe-call `?.`
  operators, `catch` blocks, labeled jumps, property accessors) as first-class, semantically-named
  rules — richer than a tree-sitter CST.
...
bigquery, mysql, sqlite, duckdb, oracle, clickhouse, redshift) with a reported confidence,
defaulting to ANSI. See [SQL metrics overview](/metrics/sql/overview) for the full set.
