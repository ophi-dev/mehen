...
<Steps>
  ...
  <Step title="Generate the parser modules">
    ...
    # provides antlr4-rust-gen (or set MEHEN_ANTLR_RUST_GEN)
    cargo install antlr-rust-runtime --version 0.13.0 --bin antlr4-rust-gen
    cargo xtask antlr generate <lang>
    ```

    The result lands in `crates/mehen-<lang>-parser/src/generated/`: the `<lang>_lexer.rs` and
    `<lang>_parser.rs` modules plus a `semantics.json` sidecar (emitted by the 0.13.0 generator).
  ...
  <Step title="Implement the analyzer + walker">
    ...
    (feeding it `mehen_antlr::token_views(parsed.tokens())`).
  </Step>
  <Step title="Register the analyzer">
