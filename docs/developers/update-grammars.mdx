...
The languages that go through tree-sitter (Go, C, PowerShell, plus the Markdown text extensions
used by `mehen-markdown`) are parsed via pinned `tree-sitter-<lang>` crates. Grammars change over
time and need periodic updates — both for bug fixes and to keep up with new syntax.
...
The Python, TypeScript / JavaScript / TSX / JSX, PHP, Ruby, Rust, and Kotlin analyzers do **not**
use tree-sitter; they have no `grammar.rs` and need no kind-enum regeneration. (Kotlin is
ANTLR-backed — see the note above.) Markdown's primary parser is pulldown-cmark; the tree-sitter
Markdown grammar is only used for ancillary text extensions.
