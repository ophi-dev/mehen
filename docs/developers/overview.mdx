...
- For tree-sitter grammar regeneration (Go / C / PowerShell): a Linux or macOS shell, or
  Windows via WSL.
- For ANTLR grammar regeneration (Kotlin, Java): the `antlr4-rust-gen` generator
  (`cargo install antlr-rust-runtime --features codegen --bin antlr4-rust-gen`). Only needed to
  *regenerate* parsers; a normal build uses the checked-in modules.
