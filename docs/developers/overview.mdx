...
- For tree-sitter grammar regeneration (Go / C / PowerShell): a Linux or macOS shell, or
  Windows via WSL.
- For ANTLR grammar regeneration (Kotlin): a Java runtime, an `antlr-4.13.2-complete.jar`, and the
  `antlr4-rust-gen` generator (`cargo install antlr-rust-runtime`). Only needed to *regenerate*
