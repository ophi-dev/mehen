...
| `--coverage[=PATH\|auto\|off]` | lazy | Coverage input: bare `--coverage` (or `=auto`) ingests configured reports and [discovers report files](/metrics/coverage/auto-discovery) unless `[coverage] discover = false`; `--coverage=<path>` (repeatable, `=` required) uses exactly those reports; `=off` disables coverage. When omitted, coverage loads only if a `coverage.*` threshold or a `[coverage]` config section asks for it. |
