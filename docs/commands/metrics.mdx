...
| `--coverage <PATH\|auto\|off>` | lazy | Coverage input: bare `--coverage` (or `auto`) [discovers report files](/metrics/coverage/auto-discovery); one or more paths use exactly those reports; `off` disables coverage. When omitted, coverage loads only if a `coverage.*` threshold or a `[coverage]` config section asks for it. |
