...
how much of the file recent history has moved, normalized by its size). The
two history columns are computed by walking the git history of each compared
revision. All columns are review-prioritization signals, not quality verdicts:
a rising number flags *where to look*, not *that the code is bad*.
