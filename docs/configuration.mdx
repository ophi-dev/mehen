...
and non-string-array lists are rejected with a caret into the source. The `--coverage` CLI flag
always wins over the file (`--coverage off` disables ingestion regardless of configuration), and
a configured `coverage.*` threshold triggers ingestion by itself. Full semantics:
