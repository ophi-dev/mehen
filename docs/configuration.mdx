...
The command fails with exit `1` before any analysis runs. One exception to load-time strictness:
the `sql.*` and `markdown.*` namespaces are extensible and accepted verbatim, so verify those
spellings against the [SQL](/metrics/sql/overview) and [Markdown](/metrics/markdown/overview)
metric references.
