...
`mehen metrics` still exits `1` because its only requested file is unavailable; `mehen diff`
records the affected side under `analysis_errors`, keeps evaluating parseable files, and is
advisory unless `--fail-on-analysis-error` is set. `mehen top-offenders` keeps its contract of
skipping unparsable files from static ranking while preserving parser-independent history values.
...
`threshold_violations` array (present only when the gate fired). Per-file analyzer diagnostics use
the separate, always-present `analysis_errors` array.
...
`sql.modularit_health` is caught just like `cognitve`. The fixed `history.*` and `coverage.*`
families are enumerated exactly — a mistyped member is rejected with the family listing instead
of silently becoming a gate that can never fire.

## The `[coverage]` section

Coverage ingestion (the [`coverage.*` metric family](/metrics/coverage/overview)) is configured
by its own table:

```toml
[coverage]
reports = ["ci-artifacts/lcov.info"]  # explicit report paths (hard error if unusable)
discover = true                        # allow report auto-discovery; `true` also
                                       # opts the run in without a --coverage flag
extra-patterns = ["qa/**/*.lcov"]      # additive discovery globs, repo-root-relative
stale-warning = true                   # warn when a report predates the newest commit
```

All keys are optional and validated like everything else — unknown keys, non-boolean toggles,
and non-string-array lists are rejected with a caret into the source. Flag/file precedence, per
mode: `--coverage=off` disables ingestion unconditionally; `--coverage=<path>` uses exactly the
supplied reports (configured `reports` and discovery are skipped); bare `--coverage` (or
`=auto`) ingests the configured `reports` and scans only when `discover` is not `false` — the
flag forces ingestion *on*, it does not override a configured discovery opt-out. A configured
`coverage.*` threshold triggers ingestion by itself. Full semantics:
[coverage auto-discovery](/metrics/coverage/auto-discovery).

Coverage thresholds are **minimums** (the family is higher-is-better):

```toml
[thresholds]
"coverage.line" = 80     # fail any measured file below 80% line coverage
"coverage.branch" = 60
...
A file *unmeasured* by every ingested report skips coverage gates entirely — "no data" is never
treated as 0%, so a path-matching hiccup cannot fail your build; only genuinely
instrumented-but-uncovered code can.
...
| `mehen diff --fail-on-analysis-error` | Inside the binary, diff only | Error/fatal diagnostics on either analyzed side | 1 |
...
- [Coverage metrics](/metrics/coverage/overview) — the `[coverage]` section and coverage gates.
