...
directory and walking up to the filesystem root. The first file found wins, so a config at the
repository root applies from any subdirectory. When both names exist in the same directory,
`mehen.toml` is used and a warning is logged.
...
| Higher is better | `mi.*` and quality scores such as `sql.maintainability_index` | Minimum | measured value < limit |
...
Metric names are the same ones `--metric` accepts: the source-code families (`cognitive`,
`cyclomatic`, `loc.*`, `halstead.*`, `mi.*`, `abc`, `nargs`, `nexit`, `nom.*`, `npa.*`, `npm.*`,
`wmc`, including aggregate spellings such as `cognitive.max`), the fixed
[history family](/metrics/history/overview) (`history.hotspot`, `history.churn.relative`, …), and
the namespaced [SQL](/metrics/sql/overview) and [Markdown](/metrics/markdown/overview) families
(`sql.*`, `markdown.*`).
...
Each line names the measured value, the crossed limit, and the config table that set it, so the
fix — either direction — is one jump away. Violations are sorted by path, then metric, for
deterministic CI logs. Colors engage only on a terminal with `NO_COLOR` unset.

## Validation

The configuration is validated when it loads. Unknown metrics, unknown languages, misspelled
`history.*` keys, and non-numeric limits fail immediately with a pointer into the file:
...
mehen::config
 ...
 2 │ cognitve = 5
   ...
   ·     ╰── not a known metric
   ╰────
  help: did you mean `cognitive`? known families: abc, cognitive, cyclomatic, halstead,
        loc, mi, nargs, nexit, nom, npa, npm, wmc, plus `sql.*`, `markdown.*`, and the
        fixed `history.*` keys
```

A typo can never silently disable a gate: the command fails with exit `1` before any analysis
runs.
