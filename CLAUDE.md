...
- **Approachable messaging**: errors and warnings must be understandable by users without deep knowledge of the metric's theory — prefer concise yet explanatory wording over jargon or bare codes.
- **Schoolbook docs**: each metric's page under `docs/metrics/` maintains a scientific yet learning-oriented style, with high-quality papers cited in its `## References` section (see `docs/metrics/code/cognitive.mdx` for the pattern).
